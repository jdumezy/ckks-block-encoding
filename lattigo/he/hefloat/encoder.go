...
func NewEncoder(parameters Parameters, precision ...uint) (ecd *Encoder) {
	...
	ecd = &Encoder{
		...
		buff:         parameters.RingQ().NewRNSPoly(),
		m:            m,
		rotGroup:     rotGroup,
	}
...
// Accepted polyOut.(type) are rlwe.Point and ring.RNSPoly.
// The imaginary part will be discarded if ringType == ring.ConjugateInvariant.
...
func (ecd Encoder) embedDouble(values FloatSlice, metadata *rlwe.MetaData, polyOut interface{}) (err error) {
	...
	default:
		return fmt.Errorf("cannot Embed: invalid polyOut.(Type) must be rlwe.Point or ring.RNSPoly but is %T", p)
	}
...
func (ecd Encoder) plaintextToComplex(level int, scale rlwe.Scale, logSlots int, p ring.RNSPoly, values FloatSlice) (err error) {
...
func (ecd Encoder) plaintextToFloat(level int, scale rlwe.Scale, logSlots int, p ring.RNSPoly, values FloatSlice) (err error) {
	if level == 0 {
...
func polyToComplexNoCRT(coeffs []uint64, values FloatSlice, scale rlwe.Scale, logSlots int, isreal bool, rQ ring.RNSRing) (err error) {

	slots := 1 << logSlots
	maxCols := int(rQ.NthRoot() >> 2)
	gap := maxCols / slots
	Q := rQ[0].Modulus
	var c uint64
...
func polyToComplexCRT(poly ring.RNSPoly, bigintCoeffs []big.Int, values FloatSlice, scale rlwe.Scale, logSlots int, isreal bool, r ring.RNSRing) (err error) {
...
func (ecd *Encoder) polyToFloatCRT(p ring.RNSPoly, values FloatSlice, scale rlwe.Scale, logSlots int, r ring.RNSRing) (err error) {
...
func (ecd *Encoder) polyToFloatNoCRT(coeffs []uint64, values FloatSlice, scale rlwe.Scale, logSlots int, r ring.RNSRing) (err error) {
