...
var (
	...
	ExampleParameters128BitLogN14LogQP438 = ParametersLiteral{
		...
		// LogQ is base 2 logarithm the ciphertext moduli, which is the product of pair-wise co-prime primes congruent to 1 modulo 2N.
		...
		// Q allows to specify the primes of Qinstead.
		//Q: []uint64{}
		...
		// LogP is an optional auxiliary modulus added on top of Q for the evaluation keys. This modulus does not contribute
		...
		// and therefor not specified here. See the description of `rlwe.EvaluationKey` and `rlwe.EvaluationKeyParameters`
		// for additional information.
		LogP: []int{55, 55},

		// P allows to specify the primes of P by their bit size instead.
		//P: []uint64{}
		...
		// Note that a homomorphic bridge between the two rings exists (see he/hefloat/bridge.go).
		...
		// See lattigo/ring/rns_sampler*.go for the available distributions and their parameters.
		Xs: &rlwe.DefaultXs,

		// Xe is the error distribution. The default value is a discrete Gaussian with standard deviation 3.2 and bounded by 19.
		// Other distributions are supported, see lattigo/ring/rns_sampler*.go for the available distributions and their parameters.
		Xe: &rlwe.DefaultXe,
	}
)
